...
export const ASK_FOR_CLARIFICATION_TOOL_NAME = "ask_for_clarification";
...
export function buildItemsForQuestion(question: QuestionData): WrappingSelectItem[] {
	...
	for (const kind of sentinelsToAppend(question)) {
		items.push({
			kind,
			label: displayLabel(kind),
			...(kind === "edit" ? { description: "Enter the number of the option you want to revise." } : {}),
		});
	}
	return items;
}
...
export const DEFAULT_PROMPT_GUIDELINES: string[] = [
	`Use ask_for_clarification whenever the user's request is underspecified and you cannot proceed without concrete decisions — you can ask up to ${MAX_QUESTIONS} questions per invocation.`,
	`Each question MUST have ${MIN_OPTIONS}-${MAX_OPTIONS} options. Every option requires a concise label (1-5 words) and a description explaining what the choice means or its trade-offs. The user can additionally type a custom answer or select the automatically appended Edit an option row to revise a choice label and description. Do NOT author "Other", "Type something.", "Edit an option", or "Next" labels yourself — reserved labels are rejected at runtime.`,
	...
	"Do not stack multiple ask_for_clarification calls back-to-back — group all clarifying questions into one invocation.",
];
...
Usage notes:
- Users can type a custom answer via the automatically appended "Type something." row or edit a model-provided option's label and description via the automatically appended "Edit an option" row. Do NOT author reserved control labels yourself.
...
export function registerAskForClarificationTool(pi: ExtensionAPI): void {
	const guidance = validateGuidanceFields(loadConfig().guidance);
	pi.registerTool({
		name: ASK_FOR_CLARIFICATION_TOOL_NAME,
			...
			if (collapseKey !== "off" && typeof ctx.ui.onTerminalInput === "function") {
				removeOverlayInputListener = ctx.ui.onTerminalInput((data) => {
					...
					if (handle.isHidden() && !hasAnnouncedHide) {
						...
						ctx.ui.notify?.(`ask_for_clarification hidden — press ${collapseKey} to reopen`, "info");
					}
					return { consume: true };
				});
			}
						...
						const session = new QuestionnaireSession({
							...
							editOption: async (option) => {
								try {
									...
									if (!editorCommand) throw new Error("No external editor command is configured");
									const initial = `Label:\n${option.label}\n\nDescription:\n${option.description}`;
									const edited = await editWithExternalEditor(tui, editorCommand, initial);
									if (edited === undefined) return undefined;
									const match = edited.match(/^Label:\s*\n?([\s\S]*?)\n\s*Description:\s*\n?([\s\S]*)$/);
									if (!match) {
										ctx.ui.notify("Keep the Label: and Description: headings when editing an option.", "warning");
										return undefined;
									}
