...
The complete programmatic surface of `ask_for_clarification`: what the model sends, what
...
```ts
import { ASK_USER_PROMPT_EVENT, type AskUserPromptEventPayload } from "@juicesharp/custom-ask-for-clarification/events";
...
The channel name is `custom-clarification:prompt`. Preview *content* is deliberately not shipped
...
as a new channel (e.g. `custom-clarification:prompt.v2`) rather than a version field.
