...
| --- | --- | --- |
| Interactive terminal | `ask_for_clarification` in its tool list | The full tabbed TUI overlay |
| RPC / ACP host (VS Code pendant, Zed, Paseo) | `ask_for_clarification` in its tool list | A sequence of the host's own native select and input dialogs |
...
turn. When there is no UI, `ask_for_clarification` is stripped from the list so the model never
