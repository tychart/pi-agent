...
```
~/.config/custom-ask-for-clarification/config.json
```
...
```json
{
  ...
  "guidance": {
    ...
    "promptGuidelines": [
      "Batch every clarifying question into one ask_for_clarification call.",
...
1. `$XDG_CONFIG_HOME/custom-ask-for-clarification/config.json`, if `XDG_CONFIG_HOME` is set,
...
2. If that file does not exist, the legacy path `~/.config/custom-ask-for-clarification/config.json`
...
`ask_for_clarification` tool — the text the model reads when deciding how to use it. There is
...
`ask_for_clarification`. Use them to make the model ask more or less often, or to enforce a
