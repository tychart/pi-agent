...
function buildSingleSelectAnswer(state: QuestionnaireState, runtime: QuestionnaireRuntime): QuestionAnswer | null {
	...
	if (!item) return null;
	if (item.kind === "other" || item.kind === "edit") {
		return null;
	}
	if (item.kind === "next") {
		return null;
...
export function routeKey(data: string, state: QuestionnaireState, runtime: QuestionnaireRuntime): QuestionnaireAction {
	...
	if (state.inputMode) {
		...
		if (isConfirm(kb, data)) {
			if (runtime.currentItem?.kind === "edit") {
				const optionIndex = Number.parseInt(runtime.inputBuffer.trim(), 10) - 1;
				return Number.isInteger(optionIndex) && optionIndex >= 0 && optionIndex < runtime.questions[state.currentTab]!.options.length
					? { kind: "edit_option", optionIndex }
					: { kind: "ignore" };
			}
