...
export interface QuestionnaireSessionConfig {
	...
	/** Edit one model-provided option outside the overlay, returning its replacement fields. */
	editOption: (option: { label: string; description: string }) => Promise<{ label: string; description: string } | undefined>;
...
export class QuestionnaireSession {
	...
	private readonly editOption: QuestionnaireSessionConfig["editOption"];
	...
	constructor(config: QuestionnaireSessionConfig) {
		...
		this.editOption = config.editOption;
	...
	private runEffect(effect: Effect): void {
		switch (effect.kind) {
			...
			case "clear_input_buffer":
				...
				return;
			case "edit_option":
				this.runOptionEdit(effect.optionIndex);
				return;
	 ...
	 */
	private runOptionEdit(optionIndex: number): void {
		const question = this.questions[this.state.currentTab];
		const option = question?.options[optionIndex];
		if (!question || !option) return;
		void this.editOption({ label: option.label, description: option.description }).then((updated) => {
			if (!updated || updated.label.trim().length === 0) return;
			option.label = updated.label.trim();
			option.description = updated.description.trim();
			(option as typeof option & { editedDescription?: string }).editedDescription = option.description;
			const item = (this.itemsByTab[this.state.currentTab] as WrappingSelectItem[] | undefined)?.[optionIndex];
			if (item?.kind === "option") {
				item.label = option.label;
				item.description = option.description;
			}
			this.state = { ...this.state, optionIndex, inputMode: false };
			this.viewAdapter.apply(this.state);
		});
	}
