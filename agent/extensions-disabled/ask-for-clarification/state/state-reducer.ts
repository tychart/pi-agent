...
export type Effect =
	...
	| { kind: "edit_option"; optionIndex: number }
...
const editOptionHandler: Handler<"edit_option"> = (state, action, _ctx) => ({
	state: { ...state, inputMode: false },
	effects: [{ kind: "clear_input_buffer" }, { kind: "edit_option", optionIndex: action.optionIndex }],
});
...
const confirmHandler: Handler<"confirm"> = (state, action, ctx) => {
	...
	if (answer.kind === "option" && answer.answer) {
		...
		const editedDescription = (matched as (typeof matched & { editedDescription?: string }) | undefined)?.editedDescription;
		if (editedDescription) answer = { ...answer, editedDescription };
	}
