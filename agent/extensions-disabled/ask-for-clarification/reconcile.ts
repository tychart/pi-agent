/**
 * reconcile — mid-session lifecycle reconciliation for ask_for_clarification.
 *
 ...
 * (packages/rpiv-advisor/advisor/handlers.ts), simplified: ask_for_clarification's
...
import { ASK_FOR_CLARIFICATION_TOOL_NAME } from "./ask-for-clarification.js";

/**
 * Strip-or-restore `ask_for_clarification` to match `ctx.hasUI`. Reads the active
 ...
 */
export function reconcileAskForClarificationTool(pi: ExtensionAPI, ctx: ExtensionContext): void {
	const active = pi.getActiveTools();
	const hasTool = active.includes(ASK_FOR_CLARIFICATION_TOOL_NAME);
	...
	if (!ctx.hasUI && hasTool) {
		pi.setActiveTools(active.filter((n) => n !== ASK_FOR_CLARIFICATION_TOOL_NAME));
	} else if (ctx.hasUI && !hasTool) {
		pi.setActiveTools([...active, ASK_FOR_CLARIFICATION_TOOL_NAME]);
	}
}
 ...
 */
export function registerAskForClarificationReconciler(pi: ExtensionAPI): void {
	pi.on("before_agent_start", (_event, ctx) => reconcileAskForClarificationTool(pi, ctx));
}
